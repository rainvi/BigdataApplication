...
<?php
   ...
   $rate = $_GET['rate']; //작성한 리뷰


   //insert 문, update 문에 prepared statement 사용

   //UPDATE 문
   $sql_rate_update = "UPDATE review2 set site_rate = ? WHERE Uid = ? and Mid = ?";   //해당 영화에 해당 유저가 작성한 리뷰 업데이트


   if ($stmt = mysqli_prepare($mysqli, $sql_review_update)){
      mysqli_stmt_bind_param($stmt, "sss", $R2, $U, $M);

      $U = $Uid;  //uid 요청*
      $M = $Mid; //Mid 요청*
      $R2 = $_GET['rating'];   //Review 요청*

      if (mysqli_stmt_execute($stmt)){
         echo "<script>alert('평점 업데이트 성공적입니다'); location.href='./movie_details.php?Mid=".$Mid."'</script>";
      } else{
         echo "ERROR : 쿼리를 실행할 수 없어요 : $sql_rate_update . ".mysqli_error($mysqli);
      }
