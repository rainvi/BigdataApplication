...
<?php
   ...
   //INSERT 문
   $sql_review_insert = "INSERT INTO review2 (Uid, Mid, Text) VALUES (?,?,?)";   //리뷰를 입력 (평점 제외, id는 자동 저장.)


   //prepared statement insert 실행
   if ($stmt = mysqli_prepare($mysqli, $sql_review_insert)){

      mysqli_stmt_bind_param($stmt, "sss", $U, $M, $R);
      ...
      if (mysqli_stmt_execute($stmt)){
         echo "<script>alert('리뷰 등록이 성공적입니다'); location.href='./movie_details.php?Mid=".$Mid."'</script>";
      } else{
